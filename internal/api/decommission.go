...
// replayCorpus builds the corpus a decommission replay reads, the same
// disk-then-memory decision handleDefinitionsReplay makes -- kept
// identical deliberately, so the two receipts an operator sees are
...
func (s *Server) replayCorpus() engine.Corpus {
	...
	if s.History != nil {
		return engine.NewRetainedCorpus(s.Store, s.History)
	}
