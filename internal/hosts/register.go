...
// instead: write-behind and rate-limited (see persistLocked), never a
// disk write on the ingest path.
package hosts
...
// main.go's ingestOneRecovered). One mutex-protected map update and,
// at most once per persistMinInterval, one JSON encode -- never a disk
// write.
...
// through persist.WriteBehind, the write. A var rather than a const so
// a test needing every call to persist immediately can shrink it, the
// same convention internal/flags.persistMinInterval uses.
var persistMinInterval = time.Second

// persistLocked hands the current state to the write-behind writer,
// which coalesces it with whatever else is pending and writes it off
// this goroutine (see persist.WriteBehind).
//
// The extra rate limit on the *encode* is what internal/flags does not
// need and this does. flags marshals on a flag mutation; this is called
// on every ingested event, and marshalling up to MaxHosts entries per
// event -- on the ingest goroutine, under the lock -- would be a real
// cost on exactly the sustained-traffic case the whole write-behind
// design exists to keep off the hot path. So a *structural* change (a
// new host, a mark set or cleared) always encodes, and a plain refresh
// of LastSeen/Events encodes at most once per persistMinInterval.
//
// The trade-off, stated plainly: if the feed goes silent right after a
// deferred refresh, that last LastSeen bump reaches disk only when
// Flush or Close forces it. Losing it costs a few seconds of
// last-seen precision after an unclean kill, never a host and never a
// mark -- the same bounded "best-effort, not critical state" trade-off
// internal/flags' own rate limit already documents.
//
// Marshal failures are swallowed rather than surfaced to Observe's
// caller: the in-memory state, which every read goes through, stays
// correct either way, so a transient encoding problem degrades to
...
// before returning -- main's shutdown joins on this (closeStoreOnShutdown)
// so a host seen right before exit is not silently dropped. A register
// with no backend configured is a safe no-op. Not safe to call any
// mutating method after Close.
func (r *Register) Close(ctx context.Context) error {
	r.encodeDeferred()
	return r.wb.Close(ctx)
}

// encodeDeferred runs the encode persistLocked skipped, if there is
// one, so Flush and Close write the true latest state rather than the
// state as of the last un-deferred call.
