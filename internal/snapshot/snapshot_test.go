...
func TestWriteThenLoadRestoresEveryPart(t *testing.T) {
	...
	path, err := New(dir, 3, store, devices).Write(taken)
	if err != nil {
	...
	report, err := Load(dir, now, loadStore, loadDevices)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if report.Path != path {
...
func TestWrittenFileIsPrivateAndCarriesTheDocumentShape(t *testing.T) {
	...
	path, err := New(dir, 3, &testPart{name: "store", payload: `{"total":1}`}).Write(taken)
	if err != nil {
		t.Fatalf("Write: %v", err)
	}
	...
	if err := json.Unmarshal(data, &doc); err != nil {
		t.Fatalf("the written file does not parse as a Document: %v", err)
	}
...
func TestRotationKeepsExactlyKeepFilesUnderAHostileClock(t *testing.T) {
	...
	if err := os.WriteFile(futureName, futureDoc, 0o600); err != nil {
		t.Fatalf("planting the hostile snapshot: %v", err)
	}

	const keep = 3
	w := New(dir, keep, &testPart{name: "store", payload: `{"generation":"real"}`})
	var lastReal string
	for i := 0; i < 6; i++ {
	...
	report, err := Load(dir, base.Add(10*time.Minute), part)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
...
func TestLoadSkipsATruncatedFileAndFallsThroughToTheNextNewest(t *testing.T) {
	...
	w := New(dir, 5, &testPart{name: "store", payload: `{"generation":"older"}`})
	older, err := w.Write(base)
	if err != nil {
		t.Fatalf("Write older: %v", err)
	}
	newer, err := New(dir, 5, &testPart{name: "store", payload: `{"generation":"newer"}`}).Write(base.Add(time.Minute))
	if err != nil {
	...
	part := &testPart{name: "store"}
	report, err := Load(dir, base.Add(2*time.Minute), part)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
...
func TestLoadSkipsAForeignSchemaAndFallsThroughToTheNextNewest(t *testing.T) {
	...
	older, err := New(dir, 5, &testPart{name: "store", payload: `{"generation":"older"}`}).Write(base)
	if err != nil {
		t.Fatalf("Write older: %v", err)
	}
	...
	if err := os.WriteFile(filepath.Join(dir, fileName(foreign)), []byte(body), 0o600); err != nil {
		t.Fatalf("planting the foreign snapshot: %v", err)
	}

	report, err := Load(dir, base.Add(2*time.Minute), &testPart{name: "store"})
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if report.Path != older {
...
func TestLoadRefusesAnExpiredSnapshot(t *testing.T) {
	...
	if _, err := New(dir, 5, &testPart{name: "store", payload: `{}`}).Write(taken); err != nil {
		t.Fatalf("Write: %v", err)
	}

	part := &testPart{name: "store"}
	_, err := Load(dir, taken.Add(MaxAge+time.Minute), part)
...
func TestLoadOnAMissingDirectoryIsAColdStartNotAFailure(t *testing.T) {
	report, err := Load(filepath.Join(t.TempDir(), "never-written"), time.Now(), &testPart{name: "store"})
	if !errors.Is(err, ErrNoSnapshot) {
...
func TestForeignFilesAreNeitherLoadedNorRotatedAway(t *testing.T) {
	...
	w := New(dir, 1, &testPart{name: "store", payload: `{}`})
	for i := 0; i < 3; i++ {
	...
	if _, err := Load(dir, base.Add(3*time.Minute), &testPart{name: "store"}); err != nil {
...
func TestAPartThatCannotExportIsOmittedRatherThanFailingTheWrite(t *testing.T) {
	...
	if _, err := New(dir, 2, broken, good).Write(taken); err != nil {
	...
	loadStore := &testPart{name: "store"}
	report, err := Load(dir, taken.Add(time.Minute), loadEngine, loadStore)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
...
func TestKeepIsFlooredAtOneSoRotationNeverDeletesWhatItJustWrote(t *testing.T) {
	...
	path, err := New(dir, 0, &testPart{name: "store", payload: `{}`}).Write(taken)
	if err != nil {
		t.Fatalf("Write: %v", err)
	}
...
func TestLoadRefusesADocumentWithNoTakenTime(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, fileName(base)), []byte(body), 0o600); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	if _, err := Load(dir, base.Add(time.Minute), &testPart{name: "store"}); !errors.Is(err, ErrNoSnapshot) {
...
func TestASnapshotWithinTheClockSkewToleranceIsStillLoaded(t *testing.T) {
	...
	if _, err := New(dir, 2, &testPart{name: "store", payload: `{}`}).Write(taken); err != nil {
		t.Fatalf("Write: %v", err)
	}

	// The loader's clock a few seconds behind the writer's: an NTP nudge,
	// not a hostile file.
	if _, err := Load(dir, taken.Add(-5*time.Second), &testPart{name: "store"}); err != nil {
