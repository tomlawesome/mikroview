...
func TestShippedPortScan_FieldsRefireClearRevive(t *testing.T) {
	...
	if _, ok, _ := fs.SetVerdict(f2.ID, flags.VerdictChecked, "operator", "", t0.Add(15500*time.Millisecond)); !ok {
...
func TestShippedCriticalPort_FieldsRefireClearRevive(t *testing.T) {
	...
	// Clear + revive.
	if _, ok, _ := fs.SetVerdict(f2.ID, flags.VerdictChecked, "operator", "", t0.Add(6*30*time.Second)); !ok {
...
func TestShippedRepeatedDrops_FieldsRefireClearRevive(t *testing.T) {
	...
	// Clear + revive.
	if _, ok, _ := fs.SetVerdict(f2.ID, flags.VerdictChecked, "operator", "", t0.Add(11*time.Minute)); !ok {
		t.Fatal("expected Clear to succeed")
	}
...
func TestShippedDistributedBruteForce_FieldsRefireClearRevive(t *testing.T) {
	...
	// Clear + revive.
	if _, ok, _ := fs.SetVerdict(f2.ID, flags.VerdictChecked, "operator", "", t0.Add(11*10*time.Second)); !ok {
		t.Fatal("expected Clear to succeed")
	}
