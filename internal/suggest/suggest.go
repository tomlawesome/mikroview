...
const (
	...
	KindPort Kind = "port"
	// KindAddressList is reserved for a future slice: suggesting a watch
	// over traffic from an address list a rule already references. Not
	// yet generated by anything -- the watchlist matching engine has no
	// live "is this address currently in that list" check, only a fixed
	// stored identity/destination, and building that is its own piece of
	// work (recorded on #243). Defined here so the Status/Sync machinery
