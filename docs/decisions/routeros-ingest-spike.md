...
```
{"event": "tls-established", "peer": "192.0.2.30"}
...
```
/system logging action set remote remote=192.0.2.30 remote-port=16803 remote-protocol=tls check-certificate=yes
...
Router: CHR 7.23.3, QEMU. mikroview: https://192.0.2.30:19801. Sink: http://192.0.2.30:19899
...
== /tool fetch url="http://192.0.2.30:19899/q1" http-method=post http-data="{\"hello\":\"router\"}" http-header-field="Content-Type: application/json,Authorization: Bearer spike-token" output=user as-value
/tool fetch url="http://192.0.2.30:19899/q1" http-method=post http-data="{\"hello\":\"router\"}" http-header-field="Content-Type: application/json,Authorization: Bearer spike-token" o
utput=user as-value
[admin@CHR] >
received at /q1:
...
--- 65430 bytes
== :local d ""; :for i from=1 to=6543 do={:set d ($d . "0123456789")}; /tool fetch url="http://192.0.2.30:19899/q2-65430" http-method=post http-data=$d output=user as-value
:local d ""; :for i from=1 to=6543 do={:set d ($d . "0123456789")}; /tool fetch url="http://192.0.2.30:19899/q2-65430" http-method=post http-data=$d output=user as-value
[admin@CHR] >
...
--- 65440 bytes
:local d ""; :for i from=1 to=6544 do={:set d ($d . "0123456789")}; /tool fetch url="http://192.0.2.30:19899/q2-65440" http-method=post http-data=$d output=user as-value
...
[admin@CHR] >
/tool fetch url="http://192.0.2.30:19899/q2b" upload=yes src-path=big.json http-method=post as-value
failure: only [s]ftp modes support upload
[admin@CHR] >
/tool fetch url="sftp://192.0.2.30:19822/drop/from-router.json" user=mvingest password=<REDACTED> src-path=big.json upload=yes as-value
...
== /tool fetch url="sftp://192.0.2.30:19822/drop/after-keychange.json" user=mvingest password=<REDACTED> src-path=big.json upload=yes as-value
/tool fetch url="sftp://192.0.2.30:19822/drop/after-keychange.json" user=mvingest password=<REDACTED> src-path=big.json upload=yes as-value
[admin@CHR] >
...
:put [/system script get [find name=mv-ingest] source]
:local p [:serialize to=json value={ver=[/system resource get version];ifs=[/interface print as-value]}]; /tool fetch url="http://192.0.2.30:19899/q3" http-method=post http-data=$p http-header-field="Content-Type: application/json,Authorization: Bearer sched-token" output=none
[mv-read@CHR] >
