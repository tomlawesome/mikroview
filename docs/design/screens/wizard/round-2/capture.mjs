...
//   node ../docs/design/screens/wizard/round-2/capture.mjs
...
for (const theme of ['dark', 'light']) {
  ...
  await page.goto('file://' + path.join(here, 'direction-s2-wizard.html') + qs);
  ...
  for (const scene of ['s1', 's2', 's3']) {
    ...
    await el.screenshot({ path: path.join(here, 'shots', `s2-${scene}-${theme}.png`) });
    console.log(`s2-${scene}-${theme}.png`);
  }
}
await browser.close();
