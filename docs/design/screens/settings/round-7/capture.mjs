// Screenshot round 7's scenes. Dark only -- round 30 (#708) removed the
// theme picker, so there is no light theme in the app to draw.
// Run from frontend/:
//   node ../docs/design/screens/settings/round-7/capture.mjs
...
const DIRECTIONS = [['r7', 'conditions-editor.html', ['s1', 's2', 's3', 's4']]];
...
for (const [letter, file, scenes] of DIRECTIONS) {
  ...
  for (const scene of scenes) {
    ...
    await el.screenshot({ path: path.join(here, 'shots', `${letter}-${scene}-dark.png`) });
    console.log(`${letter}-${scene}-dark.png`);
  }
}
await browser.close();
