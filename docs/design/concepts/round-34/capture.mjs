...
await page.goto('file://' + path.join(here, 'verdicts-exclusions.html'));
...
// 1. resting: round 30's flags tab, the first drawer open as it ships, with the verdict trio at the right of its verbs
await take('flags');

// 2. called noise: the port scan dims in place, undo where the caret was
await page.locator('tr[data-d="d2"]').click();
await page.locator('#d2 .v[data-v="noise"]').click();
await take('called-noise');

// 3. called expected, then undone
await page.locator('tr[data-d="d3"]').click();
await page.locator('#d3 .v[data-v="expected"]').click();
await take('called-expected');
await page.locator('tr[data-d="d3"] .vdone a').click();
await take('undone');

// 4. called real: stays open, says so
await page.locator('#d1 .v[data-v="real"]').click();
