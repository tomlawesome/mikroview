...
//   node ../docs/design/concepts/round-50/capture.mjs
...
for (const id of ids) {
  await shoot(id, id);
  /* the fold, opened: the same scene, the list card beside it */
  if (id === 'more-tunnels') {
    await page.evaluate(() => window.openFold(true));
    await page.waitForTimeout(300);
    await shoot(id, 'more-tunnels-open');
    await page.evaluate(() => window.openFold(false));
  }
}
await browser.close();
