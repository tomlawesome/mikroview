...
func storeFailureAdvice(e *storeUnusable) []string {
	...
	lines := []string{
		...
		"",
		fmt.Sprintf("Mikroview is running as uid %d, gid %d.", uid, gid),
	}

	if fi, err := os.Stat(e.Dir); err == nil {
		if st, ok := fi.Sys().(*syscall.Stat_t); ok {
			lines = append(lines, fmt.Sprintf(
				"%s is owned by uid %d, gid %d with mode %04o.",
				e.Dir, st.Uid, st.Gid, fi.Mode().Perm()))
		}
	} else {
		lines = append(lines, fmt.Sprintf("%s could not be read at all: %v", e.Dir, unwrapPathErr(err)))
	}
