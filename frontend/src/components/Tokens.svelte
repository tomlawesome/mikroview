<script lang="ts">
  ...
  // let one router push its state, scoped to that device. #548's page,
  // successor to TokensOverlay.svelte -- the modal retired wholesale
  // (see the PR notes: it opened only from an admin-only rail row, so
  // this page carries the same reach today).
  ...
  onMount(() => {
    ...
    fetchDevices()
      ...
      .catch(() => {
        // The page still works without the pick-list; ingest creation
        // just has nothing to offer until devices load.
        knownDevices = []
      })
  })
...
<div class="page scrollbar">
  <!-- No readOnly chip: this page stays admin-only in the rail (GET
       /api/tokens is server-gated the same way -- see
       internal/api/tokens.go's callerIsAdmin check), so a viewer never
      ...
      {#if knownDevices.length > 0}
        <select class="device-select" bind:value={device} required aria-label="Device the token speaks for">
          ...
          {#each knownDevices as d (d.id)}
            <option value={d.id}>
              {d.name && d.name !== d.id ? `${d.name} (${d.id})` : d.id}{d.configured ? '' : ' -- not in config.yaml'}
            </option>
          {/each}
        </select>
      {:else}
