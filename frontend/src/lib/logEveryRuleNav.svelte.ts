...
//
// The topography's own way in to Log every rule (#435 decision 2; the
// page was "Tune logging" until #1134 renamed it): a dark connection in
// the coverage lens is the thing prompting the page, so clicking one
// hands off which device and boundary-pair key it was clicked on. Same
// idiom as lib/topologyNav.svelte.ts's pendingDescend
...
export interface PendingLogEveryRule {
  device: string
  /** PolicyEdge.key form, `${from}|${to}` -- the dark pair that was clicked. */
  boundaryKey: string
}

class LogEveryRuleNavState {
  pending = $state<PendingLogEveryRule | null>(null)
  ...
  consume(): PendingLogEveryRule | null {
...
export const logEveryRuleNavState = new LogEveryRuleNavState()
