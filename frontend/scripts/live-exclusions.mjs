...
// Everything below -- clearing it, finding it on Exclusions, filtering,
// removing it -- has nothing to assert against when the flag itself
// never reached the server. Running it anyway used to crash the scenario
// on the first Playwright locator timeout instead of reporting the real,
// upstream reason (#450).
if (raised.ok) {
