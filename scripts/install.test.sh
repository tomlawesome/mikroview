...
stub_bin() {
  ...
  mkdir -p "$dir"
  for real in hostname awk sleep; do
    ln -s "$(command -v "$real")" "$dir/$real"
  done
  # A fake curl that always answers healthz on the first try -- nothing
  # in this suite ever really listens on the ports install.sh prints, so
  # the real curl would sit through all 30 of install.sh's retries.
  cat >"$dir/curl" <<'EOF'
#!/bin/sh
exit 0
EOF
