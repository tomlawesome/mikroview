...
sftp_start
ros "/tool fetch url=\"sftp://$HOST_ADDR:$SFTP_PORT/drop/from-router.json\" user=mvingest password=<REDACTED> src-path=big.json upload=yes as-value" | tail -3
...
ros "/tool fetch url=\"sftp://$HOST_ADDR:$SFTP_PORT/drop/after-keychange.json\" user=mvingest password=<REDACTED> src-path=big.json upload=yes as-value" | tail -3
