...
// retainedEventsStore is a store name `-restore` recognizes that
// backedUpStores never produces and excludedFromBackup never has to
// document, because ordinary `-backup` never collects it: the retained
// corpus stays out of every backup mikroview itself writes, per
// docs/decisions/event-retention.md rule 6 (#394) -- that decision is
// about what `-backup` gathers off a live deployment, and is untouched
// here.
//
// What #959 adds is a controlled way for `-restore` to accept a
// purpose-built envelope carrying history that never went through
// ingest -- the demo's seven nights, generated by cmd/demo-history and
// loaded through `-restore` rather than by faking a live feed (see that
// issue's decision: "restore it, do not ingest it"). `-restore` is
// already an explicit, operator-invoked, --force-gated overwrite of this
// deployment's own state, so accepting one more named store there is a
// narrower door than reopening what `-backup` collects.
//
// The value is plain JSON (a []struct{ Event store.Event; ReceivedAt
// time.Time }, field tags "e"/"r" -- see cmd/demo-history's
// retainedRecord, which produces this store and is kept in step with
// runRestore's decoding by comment on both sides, not by a shared type),
// not the encrypted on-disk framing internal/retention/file.go writes:
// sealing it is this deployment's own job, done with this deployment's
// own key on the way in -- see runRestore's handling below -- the same
// division #853's amendment draws for every other store here ("the
// backup envelope stays plain, readable JSON -- the tools run with the
// key available").
const retainedEventsStore = "retained_events"

// retainedEventRecord is retainedEventsStore's per-event shape. See that
// constant's doc comment for why this is a small, independently-defined
// type rather than an import of internal/retention's own unexported
// record -- the two are wire-compatible by convention, not by sharing
// Go code.
...
func runRestore(args []string) int {
	...
	// retainedEventsStore is pulled out and validated here, before the
	// known-store loop below, for two reasons: it must not trip that
	// loop's "unknown store" refusal (backedUpStores never lists it -- see
	// its own doc comment), and its own decode failure must be caught in
	// the same fully-validated-before-anything-is-touched pass everything
	// else here gets.
	var retainedEvents []retainedEventRecord
	haveRetainedEvents := false
	if raw, ok := env.Stores[retainedEventsStore]; ok {
		haveRetainedEvents = true
		delete(env.Stores, retainedEventsStore)
		if err := json.Unmarshal(raw, &retainedEvents); err != nil {
			logger.Error(fmt.Sprintf("backup: store %q did not decode: %v -- nothing has been changed", retainedEventsStore, err))
			return 1
		}
		if ourKey == nil {
			logger.Error(fmt.Sprintf("this backup carries retained corpus history (store %q) but no "+
				"history.keyFile is configured on this deployment -- refusing to write custody data to "+
				"disk with nothing to encrypt it under. Nothing has been changed", retainedEventsStore))
			return 1
		}
	}
	...
	// The retained corpus, if the envelope carried one: sealed on the way
	// in through internal/retention.Store, the same encrypted per-day
	...
	if haveRetainedEvents {
		...
		rs, err := retention.Open(retention.Options{Dir: dir, Key: ourKey, Days: cfg.History.Days, MaxBytes: cfg.History.MaxBytes})
		if err != nil {
			logger.Error(fmt.Sprintf("opening %s for the retained corpus: %v", dir, err))
			return 1
		}
		for _, rec := range retainedEvents {
			ev := rec.Event
			ev.ReceivedAt = rec.ReceivedAt
			rs.Append(ev)
		}
		// Close flushes whatever Append buffered and stops the flusher --
		// see retention.Store.Close's own doc comment. No separate Flush
		// call is needed first.
		if err := rs.Close(); err != nil {
			logger.Error(fmt.Sprintf("writing the retained corpus to %s: %v", dir, err))
			return 1
		}
